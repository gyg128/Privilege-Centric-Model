...
if [ $space = "y" ] || [ $space = "Y" ];then
    case $model in
        ...
        "7")
            ...
            ;;
        "8")
            echo "Start Attack8_tcr model checking..."
            $probcli_path Attack8_tcr_Domain.mch $model_check $m2dot Attack8_tcr_Domain.dot
            echo "Generating the state space..."
            $dot_path $dot2pdf Attack8_tcr_Domain.dot -o Attack8_tcr_Domain.pdf
            ;;
        "9")
            echo "Start Attack9_ttbr model checking..."
            $probcli_path Attack9_ttbr_Domain.mch $model_check $m2dot Attack9_ttbr_Domain.dot
            echo "Generating the state space..."
            $dot_path $dot2pdf Attack9_ttbr_Domain.dot -o Attack9_ttbr_Domain.pdf
            ;;
        "10")
            echo "Start Attack10_vbar model checking..."
            $probcli_path Attack10_vbar_Domain.mch $model_check $m2dot Attack10_vbar_Domain.dot
            echo "Generating the state space..."
            $dot_path $dot2pdf Attack10_vbar_Domain.dot -o Attack10_vbar_Domain.pdf
            ;;
        "all")
            echo "Start Attack1_pt model checking..."
            ...
            $dot_path $dot2pdf Attack7_gate_Domain.dot -o Attack7_gate_Domain.pdf
            echo "Start Attack8_tcr model checking..."
            echo "Attack8_tcr" >> attacks.txt
            $probcli_path Attack8_tcr_Domain.mch $model_check $m2dot Attack8_tcr_Domain.dot >> attacks.txt
            echo "Generating the state space..."
            $dot_path $dot2pdf Attack8_tcr_Domain.dot -o Attack8_tcr_Domain.pdf
            echo "Start Attack9_ttbr model checking..."
            echo "Attack9_ttbr" >> attacks.txt
            $probcli_path Attack9_ttbr_Domain.mch $model_check $m2dot Attack9_ttbr_Domain.dot >> attacks.txt
            echo "Generating the state space..."
            $dot_path $dot2pdf Attack9_ttbr_Domain.dot -o Attack9_ttbr_Domain.pdf
            echo "Start Attack10_vbar model checking..."
            echo "Attack10_vbar" >> attacks.txt
            $probcli_path Attack10_vbar_Domain.mch $model_check $m2dot Attack10_vbar_Domain.dot >> attacks.txt
            echo "Generating the state space..."
            $dot_path $dot2pdf Attack10_vbar_Domain.dot -o Attack10_vbar_Domain.pdf
            ;;
        *)
    ...
    case $model in
        ...
        "8")
            echo "Start Attack8_tcr model checking..."
            $probcli_path Attack8_tcr_Domain.mch $model_check
            ;;
        "9")
            echo "Start Attack9_ttbr model checking..."
            $probcli_path Attack9_ttbr_Domain.mch $model_check
            ;;
        "10")
            echo "Start Attack10_vbar model checking..."
            $probcli_path Attack10_vbar_Domain.mch $model_check
            ;;
        "all")
            echo "Start Attack1_pt model checking..."
            echo "Attack1_pt" > attacks.txt
            ...
            echo "Start Attack8_tcr model checking..."
            echo "Attack8_tcr" >> attacks.txt
            $probcli_path Attack8_tcr_Domain.mch $model_check >> attacks.txt
            echo "Start Attack9_ttbr model checking..."
            echo "Attack9_ttbr" >> attacks.txt
            $probcli_path Attack9_ttbr_Domain.mch $model_check >> attacks.txt
            echo "Start Attack10_vbar model checking..."
            echo "Attack10_vbar" >> attacks.txt
            $probcli_path Attack10_vbar_Domain.mch $model_check >> attacks.txt
            ;;
        *)
            echo "Wrong parameters!"
            ;;
